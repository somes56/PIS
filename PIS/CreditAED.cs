...
namespace PIS
{
    public partial class CreditAED : Form
    {
        private int UserID = 0;

        public CreditAED(int CreditID, int LoginID)
        {
            UserID = LoginID;
            ...
            if (CreditID != 0)
            {
                lblCreditID.Text = CreditID.ToString();
                LoadCredit(CreditID);
                DisableEdit();
            }
            else
        ...
        private void CreditAED_Load(object sender, EventArgs e)
        {
            int CreditID = Convert.ToInt32(lblCreditID.Text.Trim());

            LoadCreditItems(CreditID);
        ...
        private void AdvSearchInvoice(string SearchBy)
        {
            ...
            Invoice = DBUtil.AdvSearchInvoiceCredit(SearchBy);
        ...
        {
            int CreditID = 0;
            CreditID = Convert.ToInt32("0" + lblCreditID.Text.ToString().Trim());

            try
            {
                DataTable InvoiceDetails = new DataTable();
                InvoiceDetails = DBUtil.AdvSearchInvoiceCredit(InvoiceID, CreditID);
        ...
        private void LoadCredit(int CreditID)
        {
            try
            {
                ...
                Credit = DBUtil.LoadCredit(CreditID);

                int count = Credit.Rows.Count;

                if (count != 0)
                {
                    lblCreditID.Text = Credit.Rows[0]["id"].ToString();
                    txtCrNo.Text = Credit.Rows[0]["cr_no"].ToString();
                    txtInvNo.Text = Credit.Rows[0]["inv_no"].ToString();
                    txtTtlInv.Text = Credit.Rows[0]["ttl_inv"].ToString();
                    LoadCreditItems(CreditID);
        ...
        private void LoadCreditItems(int CreditID)
        {
            try
            {
                DataTable CreditItems = new DataTable();
                CreditItems = DBUtil.LoadCreditItems(CreditID);

                dgvInv.DataSource = CreditItems;
            }
            catch (Exception e)
            {
        ...
        private void btnPrintInvoice_Click(object sender, EventArgs e)
        {
            ...
            CreditID = Convert.ToInt32(lblCreditID.Text.ToString().Trim());
            Amount = lblAmountInWords.Text.ToString().Trim();

            if (dgvInv.Rows.Count != 0)
            {
                InvoiceCreditSht CreditSht = new InvoiceCreditSht(CreditID, Amount, UserID);
                CreditSht.ShowDialog();
            }
            else
            {
                MessageBox.Show("No Credit Item Added To Generate Credit Note", "Warning");
                return;
            }
        }
        ...
        {
            int ID = Convert.ToInt32(lblCreditID.Text.ToString().Trim());
            var CreditNo = txtCrNo.Text.ToString().Trim();
            ...
            if (ID == 0)
            {
                CreditNo = DBUtil.GenerateCreditNo(IssuePort);

                if (CreditNo != "")
                {
                    rtn = DBUtil.UpsertCredit(ID, CreditNo, InvoiceID, Uuser);

                    if (rtn == "OK")
                    {
                        ID = DBUtil.GetCreditID(CreditNo);
                        ...
                        {
                            MessageBox.Show("System Unable To Load Data For " + CreditNo, "Error");
                            return;
                        }
                    }
                    else
                ...
                {
                    MessageBox.Show(CreditNo, "Error");
                    return;
                }
            }
            else
        ...
        private void btnDel_Click(object sender, EventArgs e)
        {
            ...
            DialogResult dlgResult = MessageBox.Show("Are you sure?", "Delete this Credit Note?", MessageBoxButtons.YesNo, MessageBoxIcon.Question);

            if (dlgResult == DialogResult.Yes)
            {
                rtn = DBUtil.DelCredit(CreditID);

                if (rtn != "ERROR")
                {
                    rtn = DBUtil.DelAllCreditItem(CreditID);
                    this.Close();
                }
                else
                {
                    MessageBox.Show("System Unable To Delete Credit Note : " + txtCrNo.Text.ToString(), "Error");
                    return;
                }
            }
            else
        ...
        private void btnDeleteItem_Click(object sender, EventArgs e)
        {
            ...
            rtn = DBUtil.DelCreditItems(ItemID);
            ...
            {
                MessageBox.Show("System Unable To Delete Selected Credit Item", "Error");
                return;
            }
        }
        ...
        private void btnAddItems_Click(object sender, EventArgs e)
        {
            ...
            rtn = DBUtil.UpsertCreditItems(ItemID, CreditID, ItemCodeID, Qty, UnitPrice, Amount, Uuser);
