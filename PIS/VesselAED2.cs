...
namespace PIS
{
    public partial class VesselAED2 : Form
    {
        private int row_id = 0;
        private string dbtblname = "M_Vessel";
        private int UserID = 0;

        public VesselAED2(int mode, int id, int LoginID)
        {
            UserID = LoginID;
            InitializeComponent();
            ...
            if (mode == 1)
            {
                ...
                LoadCustomerList(cbPortOperator, "NONE");
                LoadCustomerList(cbShippingAgent, "NONE");
                LoadCustomerList(cbPSA, "NONE");
                tbVessel_Id.Focus();
            }
            else
            {
        ...
        private void LoadCustomerList(ComboBox cb,string searchq)
        {
            ...
            try
            {
                ...
                SqlCommand cmd = new SqlCommand("SELECT id, name FROM M_CompanyInfo WHERE name LIKE '%" + searchq + "%'", con);
                SqlDataReader dtr;
                con.Open();
        ...
        private void EnableEdit()
        {
            ...
            tbSPSA.ReadOnly = false;
            tbSSA.ReadOnly = false;
            ...
            tbSSA.Text = tbShippingAgent.Text;
            tbSPO.Text = txtPortOperator.Text;
            tbSPSA.Text = tbPSA.Text;
            LoadCustomerList(cbPortOperator, txtPortOperator.Text);
            LoadCustomerList(cbPSA, tbPSA.Text);
            LoadCustomerList(cbShippingAgent, tbShippingAgent.Text);
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                if (row_id == 0)
                {
                    if (DBUtil.isExistinDB(dbtblname, "vid", tbVessel_Id.Text))
                    {
                        ...
                        MessageBox.Show("Vessel ID already exist, try another");
                        tbVessel_Id.Focus();
                        return;
                    }
                }
            ...
            if (tbVesselName.TextLength == 0)
            {
                ...
                MessageBox.Show("Vessel Name must be entered");
                tbVesselName.Focus();
                return;
            }

            if(cbShippingAgent.SelectedIndex==-1)
            {
                MessageBox.Show("Shipping Agent not specified,type NONE if not available");
                tbSSA.Text = "NONE";
                tbSSA.Focus();
            ...
            if (cbPSA.SelectedIndex == -1)
            {
                MessageBox.Show("PSA not specified,type NONE if not available");
                tbSPSA.Text = "NONE";
                tbSPSA.Focus();
            ...
            if (cbPortOperator.SelectedIndex == -1)
            {
                MessageBox.Show("Port Operator not specified,type NONE if not available");
                tbSPO.Text = "NONE";
                tbSPO.Focus();
        ...
        private void TbSPO_Leave(object sender, System.EventArgs e)
        {
         

          LoadCustomerList(cbPortOperator, tbSPO.Text);
          cbPortOperator.Focus();
          cbPortOperator.DroppedDown = true;

        }

        private void tbSPSA_Leave(object sender, EventArgs e)
        {
            LoadCustomerList(cbPSA, tbSPSA.Text);
            cbPSA.Focus();
            cbPSA.DroppedDown = true;
        }



        private void tbSSPA_Leave(object sender, EventArgs e)
        {
            LoadCustomerList(cbShippingAgent, tbSSA.Text);
            cbShippingAgent.Focus();
            cbShippingAgent.DroppedDown = true;
        }
    }
}
